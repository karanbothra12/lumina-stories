...
*   **Storage:** JSON (Blocks: Paragraph, Header, List, Image, Quote).
*   **Rendering:** `BlogContent.tsx` component iterates over blocks and renders clean, styled Tailwind HTML.
*   **SEO Layer:** Authors can attach custom `seoTitle`, `seoDescription`, and `seoKeywords` via the Editor sidebar. These fields flow through Prisma -> API -> `generateMetadata` to enrich OpenGraph/Twitter output.
